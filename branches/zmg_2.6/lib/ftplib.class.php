...
| License: GPL                                                        |
| Filename: ftplib.class.php                                          |
|                                                                     |
-----------------------------------------------------------------------
* @version $Id:ftplib.class.php 139 2007-03-12 09:21:44Z mikedeboer $
* @package zOOmGallery
...
class ftplib extends zoom {
    ...
    function login($user, $pass)
    {
        if ($this->_use_mod_ftp) {
    	   ...
    	   $this->putcmd("USER", $user);
    	   if (!$this->ok()) {
            return FALSE;
            }
            $this->putcmd("PASS", $pass);
    	}
    	if ($this->ok()) {
	...
	function chmod($command)
    {
	   ...
	   if (strtolower(PHP_OS) == 'winnt') return true;
	   
	    if ($this->_use_mod_ftp) {
    		ftp_site($this->_socket, 'CHMOD '.$command);
    	} else {
    	   $this->putcmd("CHMOD", $command);
    	}
    	return $this->ok();
    }
    /**
