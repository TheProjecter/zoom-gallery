...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id:ecard.class.php 106 2007-02-10 22:30:30Z kevinuru $
* @package zOOmGallery
...
class ecard extends image {
	...
	function save($imgid, $to_name, $from_name, $to_email, $from_email, $message) {
		...
		$this->_image = $imgid;
		$this->_to_name = trim($zoom->escapeString($to_name));
		$this->_from_name = trim($zoom->escapeString($from_name));
		$this->_to_email = trim($zoom->escapeString($to_email));
		$this->_from_email = trim($zoom->escapeString($from_email));
		$this->_message = trim($zoom->escapeString($message));
