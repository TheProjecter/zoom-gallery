 ...
 */
/**
* @version $Id:UnixPlatform.class.php 106 2007-02-10 22:30:30Z kevinuru $
* @package zOOmGallery
...
class UnixPlatform extends zoom {
    ...
    function mkdir($filename, $perms = '0666') {
        $perms = octdec($perms);
    	$umask = umask(0);
    	/*
     ...
     * @param $perms Integer value to chmod files. NULL = dont chmod files.
     * @return TRUE=all succeeded FALSE=one or more chmods failed
     */
    function chmod($path, $perms = '644') {
        $ret = TRUE;
        $perms = octdec($perms);
    	if (is_dir($path)) {
    		$dh = opendir($path);
