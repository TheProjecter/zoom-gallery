...
| Copyright: copyright (C) 2007 by Jetze van der Wal                  |
...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id:zoomthumb.php 106 2007-02-10 22:30:30Z kevinuru $
* @package zOOmGallery
* @author Jetze van der Wal 
**/
...
switch (strtolower($task)) {
	...
	case "step2": //step1 produced galleryid, so now list thumbnails in that gallery
		...
		echo '<p><a href="'. (($zoom->_isBackend) ? 'index2.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step1' : sefReltoAbs('index2.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step')) .'">(back to step 1:: select gallery)</a></p>';
	...
	case "step4":// show mambotcode
		...
		echo '<p><a href="'. (($zoom->_isBackend) ? 'index2.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step1' : sefReltoAbs('index.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step1')) .'">(back to step 1:: select gallery)</a></p>';
		echo '<p><a href="'. (($zoom->_isBackend) ? 'index2.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step2&catid='.$catid : sefReltoAbs('index.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step2&catid='.$catid)) .'">(back to step 2:: select image)</a></p>';
		echo '<p><a href="'. (($zoom->_isBackend) ? 'index2.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step3&imgid='.$imgid : sefReltoAbs('index.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step3&imgid='.$imgid )) .'">(back to step 3:: select styling)</a></p>';
		...
		$thumbzoomcode .= '}';
		$thumbzoomcode = stripslashes($thumbzoomcode);
	...
	default: //step 1
		...
		if(isset($zoom->_CAT_LIST)){
			foreach($zoom->_CAT_LIST as $category){
		 		echo '<p><a href="'. (($zoom->_isBackend) ? 'index2.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step2&catid='.$category['id'] : sefReltoAbs('index.php?option=com_zoom&Itemid='.$Itemid.'&page=zoomthumb&task=step2&catid='.$category['id'])) .'">Select:: </a> '.$category['catname']. '</p>';
			}
		}else{ 
...
echo '<div align="left" style="clear: both; height: 300px;"></div><a href="http://forge.joomla.org/sf/projects/zoomthumb">Zoom Thumb</a>, use your Zoom gallery images in your Joomla! content.<hr></div>';
